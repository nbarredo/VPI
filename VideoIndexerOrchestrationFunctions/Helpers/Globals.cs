...
namespace OrchestrationFunctions
{
    public class Globals
    {
        ...
        public static VippyProcessingState GetManifestStateRecord(string Id, TraceWriter log)
        {
            LogMessage(log, $" get manifest id: {Id} ");
            var collectionName = ProcessingStateCosmosCollectionName;
            var client = GetCosmosClient(collectionName);
            ...
            LogMessage(log, $"got manifest with id: {state?.AlternateId} ");
            return state;
        }



        public static VippyProcessingState GetManifestStateRecordByVideoId(string videoIndexerId, TraceWriter log)
        {
            LogMessage(log, $" get manifest VideoId: {videoIndexerId} ");
            var collectionName = ProcessingStateCosmosCollectionName;
            var client = GetCosmosClient(collectionName);
            var uri = UriFactory.CreateDocumentCollectionUri(CosmosDatabasename, collectionName);
            ...
            var state = result.FirstOrDefault();
            LogMessage(log, $"got manifest with VideoId: {state?.VideoIndexerId} ");
            return state;
        }
            ...
            string alternateId, TraceWriter log)
        {
            ...
            var state = GetManifestStateRecord(alternateId, log);
            var props = state.CustomProperties ?? new Properties();
            var jsonProps = JsonConvert.SerializeObject(props);
            LogMessage(log, $"props == {jsonProps}");
            LogMessage(log, $"videoIndexerCallbackUrl");
            ...
            LogMessage(log, $"language set to {language}");
