...
namespace OrchestrationFunctions
{
    public static class VideoIndexerCompleteQueueHandler
    {
            ...
            TraceWriter log)
        {
            ...
            Globals.LogMessage(log, $" call VI with uri{uri}");
            var response = await client.GetAsync(uri);
            var json =await response.Content.ReadAsStringAsync();
            Globals.LogMessage(log, $"get state with id: {videoIndexerVideoId}");
            var state = Globals.GetManifestStateRecordByVideoId(videoIndexerVideoId, log);
            Globals.LogMessage(log, $"create poco from json ");
            ...
            Globals.LogMessage(log, $"Process transcripts");
            ...
            Globals.LogMessage(log, $"start task extract images");
            taskList.Add(ExtractImages(videoBreakdownPoco, log));
            ...
            Globals.LogMessage(log, $"wait for tasks to finish");
            await Task.WhenAll(taskList.ToArray());

            Globals.LogMessage(log, $"store in json breakdowns");
            ...
            Globals.LogMessage(log, $"update processing ");
            var storeTask2 = UpdateProcessingStateAsync(completionData["id"]);
            Globals.LogMessage(log, $"wait for  store tasks to finish");
        ...
        /// <returns></returns>
        private static async Task ExtractImages(VideoBreakdownPOCO poco, TraceWriter log)
        {
            ...
            Globals.LogMessage(log, $"wait for task extract images to finish");
        ...
        private static async Task StoreBreakdownJsonInCosmos(VideoBreakdownPOCO videoBreakdownJson, TraceWriter log)
        {
            ...
            Globals.LogMessage(log, $"saving json: {json}");
